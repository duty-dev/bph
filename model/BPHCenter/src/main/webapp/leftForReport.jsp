          	...
          	<div> 
          		 <div class="clear">
	          		 <table class="ty-tj-table mt10 ml6">
		          		...
		          		<tr>
	          				<td colspan="3" class="ty-tj-td pd10">
	          					<div class="ty-tj-title">警情类型<span class="fr cursor" id="btnAddType1" onclick="clearAlarmTypeList()"><button class="ty-btn-qc"></button>清除</span>
	          					<span class="fr cursor" id="btnAddType1" onclick="addAlarmTypeList()"><button class="ty-btn-tj"></button>添加</span></div>
				...
				<tr>
					...
					<td>
						<style>
							...
							#ul_timespan li {float:left;width:40px;padding:5px; list-style: none;font-size:12px;}
							#ul_timespan li:hover {float:left;width:40px;padding:5px; list-style: none;cursor:pointer;background:black;font-size:13px;}
...
$(function() {
	    ...
	    if(m_pageType==1||m_pageType=="1"){
	    	m_statistic_typeId = 1;
	    	$("#btnalarmType").parent().find("span").each(function(i){$(this).removeClass("ty-btn-selected");});
	    	$("#btnalarmType span").addClass("ty-btn-selected");
	    }else{
	       	m_statistic_typeId = 2;
	       	$("#btnalarmType").parent().find("span").each(function(i){$(this).removeClass("ty-btn-selected");});
	       	$("#btnalarmOrgan span").addClass("ty-btn-selected");
	    }
		...
		$("#dpEDate").data("kendoDatePicker").enable(false); 
	 	searchAction();
	 	if(m_pageType==1){
	 		$("#h_title").text("警情统计——警情分类统计");
	   	}else if(m_pageType==2){
	 		$("#h_title").text("警情统计——机构统计");
	   	} 
	 	setH();
